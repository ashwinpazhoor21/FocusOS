...
final class SQLiteManager {
    ...
    func insertAppEvent(timestamp: Date, bundleId: String, appName: String, isIdle: Bool) {
        queue.async {
            let sql = """
            INSERT INTO app_events(timestamp, bundle_id, app_name, is_idle)
            VALUES (?, ?, ?, ?);
            """
            ...
            // timestamp as seconds since 1970 (Double)
    ...
    func fetchAppEvents(forDay day: Date) -> [AppEventRow] {
        ...
        return queue.sync {
            ...
            let sql = """
            SELECT timestamp, bundle_id, app_name, is_idle
            FROM app_events
